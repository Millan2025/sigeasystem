"use client";

import BackButton from "@/components/BackButton";

export default function ComprasPage() {
  return (
    <div className="min-h-screen bg-stone-50">
      <header className="bg-white shadow-sm p-4 flex items-center gap-3 sticky top-0 z-10">
        <BackButton />
<<<<<<< HEAD
        <h1 className="text-xl font-bold text-stone-800 flex-1">compras</h1>
      </header>
      <div className="p-4 max-w-7xl mx-auto">
        <div className="bg-white rounded-2xl p-12 text-center border border-stone-200">
          <p className="text-stone-500">MÃ³dulo de compras en construcciÃ³n</p>
          <p className="text-sm text-stone-400 mt-2">Funcionalidad disponible prÃ³ximamente</p>
=======
        <h1 className="text-xl font-bold text-stone-800 flex-1">Compras</h1>
      </header>
      <div className="p-4 max-w-7xl mx-auto">
        <div className="bg-white rounded-2xl p-12 text-center border border-stone-200">
          <p className="text-stone-500">Módulo de Compras en construcción</p>
          <p className="text-sm text-stone-400 mt-2">Funcionalidad disponible próximamente</p>
>>>>>>> main
        </div>
      </div>
    </div>
  );
}