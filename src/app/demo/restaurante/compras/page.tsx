...
export default function ComprasPage() {
  ...
  const [ComprasPage, setComprasPage] = useState<Pedido[]>([]);
  const [loading, setLoading] = useState(true);
  const [filtroEstado, setFiltroEstado] = useState<string>("todos");
  const [detallePedido, setDetallePedido] = useState<Pedido | null>(null);
  const [mensaje, setMensaje] = useState("");

  const cargarComprasPage = async (showLoading = true) => {
    if (showLoading) setLoading(true);
    try {
      const res = await fetch(`/api/ComprasPage?tenant=${tenantId}`);
      const data = await res.json();
      if (data.success) setComprasPage(data.data || []);
      else setComprasPage([]);
    } catch (e) {
      setComprasPage([]);
    }
  ...
  useEffect(() => {
    cargarComprasPage(true);
    const interval = setInterval(() => cargarComprasPage(false), 10000);
    return () => clearInterval(interval);
  }, [tenantId]);

  const cambiarEstado = async (id: string, nuevoEstado: string) => {
    const pedido = ComprasPage.find(p => p.id === id);
    if (!pedido) return;
    ...
    try {
      const res = await fetch("/api/ComprasPage", {
        method: "PUT",
  ...
  const confirmarPedido = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/ComprasPage/${id}/confirmar`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const cancelarPedido = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/ComprasPage/${id}/cancelar`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const ComprasPageFiltrados = ComprasPage.filter(p => {
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      <header className="bg-white shadow-sm p-4 flex items-center gap-3 sticky top-0 z-10">
        <BackButton />
        <h1 className="text-xl font-bold text-stone-800 flex-1">ComprasPage</h1>
        <button onClick={() => cargarComprasPage(true)} className="p-2 hover:bg-stone-100 rounded-xl">
      ...
      <div className="p-4 max-w-7xl mx-auto">
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          >
            Todos ({ComprasPage.length})
          </button>
          {LISTA_ESTADOS.map((estado) => {
            const info = ESTADOS[estado as keyof typeof ESTADOS];
            const count = ComprasPage.filter(p => p.estado === estado).length;
            return (
              <button
                key={estado}
        ...
        ) : ComprasPageFiltrados.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 text-center border border-stone-200">
            <ShoppingBag className="w-16 h-16 text-stone-300 mx-auto mb-4" />
            <p className="text-stone-500">No hay ComprasPage en este estado.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {ComprasPageFiltrados.map((pedido) => {
