...
const tenantId = tenantFromUrl || negocio?.tenantId || "7e045520-5e36-4e3f-a39f-10ea7d6dce76";
  ...
  const guardarPedido = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMensaje(`Ô£à Pedido #${data.data.id.slice(0, 6)} creado`);
        setTimeout(() => setMensaje(""), 5000);
            ...
            {pedidosFiltrados.map((pedido) => {
              ...
              return (
                <div key={pedido.id} className="bg-white rounded-2xl p-4 shadow-sm border border-stone-200 hover:shadow-md transition">
                  ...
                  <p className="text-sm text-stone-500">­ƒôª {pedido.order_items?.length || 0} productos</p>
                  <p className="text-sm text-stone-500">­ƒÆ░ ${pedido.total?.toLocaleString()}</p>
                  <p className="text-xs text-stone-400">Pago: {pedido.metodo_pago}</p>
                  {pedido.direccion_entrega && <p className="text-xs text-stone-400">­ƒôì {pedido.direccion_entrega}</p>}
