...
export default function ReportesPage() {
  ...
  const [agrupacionTemporal, setAgrupacionTemporal] = useState("dia");
  ...
  const exportarIA = () => {
    ...
    link.download = `reporte_ia_${negocioSlug}.json`;
    link.click();
  };
      ...
      <div className="p-3 sm:p-4 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 items-start sm:items-center">
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
              ...
              <input
                ...
                className="flex-1 sm:flex-none border border-stone-300 rounded-xl px-3 py-1.5 text-sm text-stone-900 bg-white"
              />
            ...
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
              ...
              <span className="text-sm font-medium text-stone-900">Método:</span>
              <select
            ...
            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
              ...
              <span className="text-sm font-medium text-stone-900">Agrupar:</span>
              <select
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 mb-6">
          <div className="bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-stone-200 text-center">
            <p className="text-[10px] sm:text-sm text-stone-500">Ventas</p>
            <p className="text-base sm:text-2xl font-bold text-emerald-600">${totalVentas.toLocaleString()}</p>
          </div>
          <div className="bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-stone-200 text-center">
            <p className="text-[10px] sm:text-sm text-stone-500">Compras</p>
            <p className="text-base sm:text-2xl font-bold text-red-600">${totalCompras.toLocaleString()}</p>
          </div>
          <div className="bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-stone-200 text-center">
            <p className="text-[10px] sm:text-sm text-stone-500">Ingresos</p>
            <p className="text-base sm:text-2xl font-bold text-blue-600">${totalIngresos.toLocaleString()}</p>
          </div>
          <div className="bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-stone-200 text-center">
            <p className="text-[10px] sm:text-sm text-stone-500">Egresos</p>
            <p className="text-base sm:text-2xl font-bold text-orange-600">${totalEgresos.toLocaleString()}</p>
          </div>
          <div className="bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-stone-200 text-center">
            <p className="text-[10px] sm:text-sm text-stone-500">Créditos Pend.</p>
            <p className="text-base sm:text-2xl font-bold text-purple-600">${totalCreditoPendiente.toLocaleString()}</p>
          </div>
          <div className="bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-stone-200 text-center">
            <p className="text-[10px] sm:text-sm text-stone-500">Stock Crítico</p>
            <p className={`text-base sm:text-2xl font-bold ${stockCritico > 0 ? "text-red-600" : "text-emerald-600"}`}>
              {stockCritico}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6">
          <div className="bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-stone-200">
            <h3 className="font-semibold text-stone-800 mb-3 text-sm sm:text-base flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-emerald-500" /> Ingresos por Categoría
            </h3>
            {ingresosPorCategoria.length === 0 ? (
              <p className="text-stone-500 text-sm text-center py-4">No hay datos disponibles</p>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                <RePieChart>
                  <Pie
                    ...
                    cy="50%"
                    outerRadius={70}
                    label={({ name, percent = 0 }) => `${name} ${((percent as number) * 100).toFixed(0)}%`}
                  >
        ...
        <div className="bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-stone-200 mb-6">
          ...
          {ventasTemporales.length === 0 ? (
            <p className="text-stone-500 text-sm text-center py-4">No hay datos</p>
          ) : (
            <ResponsiveContainer width="100%" height={200}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs sm:text-sm">
                  <thead className="bg-stone-50">
                    <tr>
                      <th className="text-left p-1 sm:p-2 text-stone-700">Producto</th>
                      <th className="text-right p-1 sm:p-2 text-stone-700">Cant.</th>
                      <th className="text-right p-1 sm:p-2 text-stone-700">Total</th>
                    </tr>
                  </thead>
                  <tbody>
                    {topProductos.map((p, i) => (
                      <tr key={i} className="border-b border-stone-100">
                        <td className="p-1 sm:p-2 text-stone-800 truncate max-w-[100px] sm:max-w-none">{p.nombre}</td>
                        <td className="p-1 sm:p-2 text-right text-stone-600">{p.cantidad}</td>
                        <td className="p-1 sm:p-2 text-right font-medium text-stone-800">${p.total.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {clientesTopCompras.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs sm:text-sm">
                  <thead className="bg-stone-50">
                    <tr>
                      <th className="text-left p-1 sm:p-2 text-stone-700">Cliente</th>
                      <th className="text-right p-1 sm:p-2 text-stone-700">Total</th>
                      <th className="text-right p-1 sm:p-2 text-stone-700">Trans.</th>
                    </tr>
                  </thead>
                  <tbody>
                    {clientesTopCompras.map((c, i) => (
                      <tr key={i} className="border-b border-stone-100">
                        <td className="p-1 sm:p-2 text-stone-800 truncate max-w-[80px] sm:max-w-none">{c.cliente}</td>
                        <td className="p-1 sm:p-2 text-right font-medium text-stone-800">${c.total.toLocaleString()}</td>
                        <td className="p-1 sm:p-2 text-right text-stone-600">{c.transacciones}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs sm:text-sm">
                  <thead className="bg-stone-50">
                    <tr>
                      <th className="text-left p-1 sm:p-2 text-stone-700">Cliente</th>
                      <th className="text-right p-1 sm:p-2 text-stone-700">Total Pagado</th>
                    </tr>
                  </thead>
                  <tbody>
                    {clientesTopPagos.map((c, i) => (
                      <tr key={i} className="border-b border-stone-100">
                        <td className="p-1 sm:p-2 text-stone-800 truncate max-w-[100px] sm:max-w-none">{c.cliente}</td>
                        <td className="p-1 sm:p-2 text-right font-medium text-stone-800">${c.totalPagado.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-stone-200">
          <h3 className="font-semibold text-stone-800 mb-3 text-sm sm:text-base">Últimas Ventas</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-xs sm:text-sm">
              <thead className="bg-stone-50">
                <tr>
                  <th className="text-left p-1 sm:p-2 text-stone-700">Fecha</th>
                  <th className="text-left p-1 sm:p-2 text-stone-700">Método</th>
                  <th className="text-left p-1 sm:p-2 text-stone-700">Total</th>
                  <th className="text-left p-1 sm:p-2 text-stone-700 hidden sm:table-cell">Productos</th>
                </tr>
              </thead>
              <tbody>
                {ventas.slice(0, 10).map((v) => (
                  <tr key={v.id} className="border-b border-stone-100">
                    <td className="p-1 sm:p-2 text-stone-800 whitespace-nowrap">{new Date(v.fecha).toLocaleDateString()}</td>
                    <td className="p-1 sm:p-2 text-stone-600">{v.metodo_pago}</td>
                    <td className="p-1 sm:p-2 text-stone-800 font-medium whitespace-nowrap">${(v.total || 0).toLocaleString()}</td>
                    <td className="p-1 sm:p-2 text-stone-600 hidden sm:table-cell max-w-[150px] truncate">
                      {(v.sale_items || []).map((i: any) => `${i.quantity} ${i.product_name || i.nombre || ""}`).join(", ")}
                    </td>
                  </tr>
                ))}
                {ventas.length === 0 && (
