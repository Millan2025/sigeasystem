...
export default function POSPage() {
  ...
  useEffect(() => {
    const getTenant = async () => {
      // 1. Priorizar tenant de URL
      if (tenantFromUrl) {
        setTenantId(tenantFromUrl);
        return;
      }
      // 2. Fallback: tenant del usuario autenticado
  ...
  // Cargar productos usando el tenantId
  useEffect(() => {
    if (!tenantId) return;
    fetch(`/api/products?tenant=${tenantId}`)
      ...
      .then(d => {
        if (d.success && d.data.length > 0) {
          ...
          console.warn('No se encontraron productos para el tenant:', tenantId);
        }
      })
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end md:items-center md:justify-center" onClick={() => setShowPay(false)}>
          <div className="bg-white w-full md:max-w-sm md:rounded-3xl rounded-t-3xl p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-3 gap-3 mb-3">
              ...
              <button onClick={() => pay('Bancolombia')} className="bg-yellow-500 text-white rounded-2xl py-5 font-bold text-base hover:bg-yellow-600">Bancolombia</button>
            </div>
