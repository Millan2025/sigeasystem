...
import { ArrowLeft, Download, BarChart3, PieChart, Star } from 'lucide-react'
    ...
    <div className="min-h-screen bg-stone-50 md:max-w-2xl lg:max-w-4xl mx-auto">
      <header className="bg-gradient-to-r from-stone-800 to-stone-700 text-white p-5">
        ...
        <div className="flex gap-1 bg-stone-700 rounded-xl p-1">
          {['hoy','semana','mes'].map(p => <button key={p} onClick={() => setPeriodo(p as any)} className={'flex-1 py-2 rounded-lg text-sm font-medium ' + (periodo === p ? 'bg-white text-stone-800' : 'text-stone-300')}>{p === 'hoy' ? 'Hoy' : p === 'semana' ? 'Semana' : 'Mes'}</button>)}
        </div>
      </header>
      <div className="p-4 space-y-4">
        <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-6 text-white">
          <p className="text-emerald-100 text-sm">VENTAS {periodo === 'hoy' ? 'DE HOY' : 'DEL PERÍODO'}</p>
        ...
        <div className="bg-white rounded-2xl p-5 border border-stone-200">
          ...
          <div className="flex items-end gap-1 h-32">
            {ventasPorHora.map(v => <div key={v.hora} className="flex-1 flex flex-col items-center gap-1"><span className="text-xs text-stone-500">${(v.ventas/1000).toFixed(0)}k</span><div className="w-full bg-emerald-500 rounded-t-lg" style={{ height: (v.ventas / maxVentaHora * 100) + '%' }}></div><span className="text-xs text-stone-400">{v.hora}</span></div>)}
          </div>
        </div>
        <div className="bg-white rounded-2xl p-5 border border-stone-200">
          <h3 className="font-bold text-stone-800 mb-3 flex items-center gap-2"><Star className="w-4 h-4 text-amber-500" /> Más Vendidos</h3>
          {topProductos.map((p, i) => <div key={p.nombre} className="flex items-center justify-between py-2 border-b border-stone-100 last:border-0"><div className="flex items-center gap-2"><span className="text-lg">{p.icono}</span><div><p className="text-sm font-medium text-stone-800">{p.nombre}</p><p className="text-xs text-stone-400">{p.ventas} unidades</p></div></div><p className="font-bold text-emerald-600">${p.total.toLocaleString()}</p></div>)}
        </div>
        <div className="bg-white rounded-2xl p-5 border border-stone-200">
          <h3 className="font-bold text-stone-800 mb-3 flex items-center gap-2"><PieChart className="w-4 h-4" /> Métodos de Pago</h3>
          {metodoPago.map(m => <div key={m.metodo} className="mb-3"><div className="flex justify-between text-sm mb-1"><span>{m.metodo}</span><span>{m.porcentaje}%</span></div><div className="w-full bg-stone-100 rounded-full h-3"><div className={m.color + ' h-3 rounded-full'} style={{ width: m.porcentaje + '%' }}></div></div></div>)}
        </div>
      </div>
    </div>
  )
