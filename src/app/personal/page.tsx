...
interface Empleado {
  id: string; nombre: string; documento: string; cargo: string; salarioBase: number;
}

const empleadosDemo: Empleado[] = [
  { id: '1', nombre: 'Carlos Gómez', documento: '12345678', cargo: 'Cajero', salarioBase: 1423500 },
  { id: '2', nombre: 'María López', documento: '87654321', cargo: 'Panadera', salarioBase: 1500000 },
]

export default function PersonalPage() {
  const [tab, setTab] = useState<'empleados'|'nomina'|'exportar'>('empleados')
  const [empleados] = useState<Empleado[]>(empleadosDemo)
  const [showAdd, setShowAdd] = useState(false)
  const totalNomina = empleados.reduce((s, e) => s + e.salarioBase, 0)

  function exportarExcel() {
    let csv = '\uFEFFEMPLEADO,DOCUMENTO,CARGO,SALARIO\n'
    empleados.forEach(e => { csv += e.nombre + ',' + e.documento + ',' + e.cargo + ',' + e.salarioBase + '\n' })
      ...
      <div className="p-4">
        {tab === 'empleados' && (
          <div className="space-y-3">
            ...
            {empleados.map(e => (
              <div key={e.id} className="bg-white rounded-2xl p-4 border"><h3 className="font-bold text-stone-900">{e.nombre}</h3><p className="text-sm text-stone-500">{e.cargo} · CC: {e.documento}</p><p className="text-lg font-bold text-emerald-600 mt-2">${e.salarioBase.toLocaleString()}</p></div>
            ))}
          </div>
        )}
        {tab === 'nomina' && (
          <div className="bg-white rounded-2xl p-4 border"><h3 className="font-bold mb-3">Nómina del Mes</h3>{empleados.map(e => <div key={e.id} className="flex justify-between py-2 border-b"><span>{e.nombre}</span><span className="font-bold text-emerald-600">${e.salarioBase.toLocaleString()}</span></div>)}<div className="flex justify-between pt-3 font-bold text-lg"><span>Total</span><span className="text-emerald-600">${totalNomina.toLocaleString()}</span></div></div>
        )}
        {tab === 'exportar' && (
