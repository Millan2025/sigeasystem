...
export default function InventarioPage() {
  ...
  useEffect(() => {
    fetch('/api/inventory' + (typeof window !== 'undefined' && window.location.search.includes('demo=fjmillan39') ? '?demo=true' : '')).then(r => r.json()).then(d => {
      if (d.success && d.data.length > 0) {
        setProductos(d.data.map((p: any) => ({
          id: p.id, nombre: p.name || p.nombre, icono: '📦',
          categoria: p.category || 'General', stock: Number(p.stock) || 0,
          stockMin: Number(p.min_stock) || 5, precio: Number(p.price) || 0,
          costo: Number(p.cost) || 0, ventasDiarias: 5, importancia: 3,
          esPeso: false, proveedor: '', diasParaAgotar: 10, unidad: 'unidad'
        })))
      }
    }).catch(() => {})
  }, [])
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="p-3 space-y-3">
        ...
        <div className="space-y-2">{filtrados.map(p => (
          <div key={p.id} className={'bg-white rounded-2xl p-4 border-2 ' + getAlertaColor(p.diasParaAgotar)}>
            <div className="flex items-start justify-between mb-2"><div className="flex items-center gap-2"><span className="text-3xl">{p.icono}</span><div><h3 className="font-semibold text-stone-800 text-sm">{p.nombre}</h3><p className="text-xs text-stone-400">{p.categoria} • {p.proveedor} • {p.unidad}</p></div></div><div className="text-right"><div className="flex items-center gap-1">{[1,2,3,4,5].map(e => <Star key={e} className={'w-3 h-3 ' + (e <= p.importancia ? 'fill-amber-400 text-amber-400' : 'text-stone-300')} />)}</div><span className={'text-xs font-bold px-2 py-0.5 rounded-full ' + getAlertaColor(p.diasParaAgotar)}>{getAlertaTexto(p.diasParaAgotar)}</span></div></div>
            <div className="grid grid-cols-4 gap-2 text-center text-xs"><div><p className="text-stone-400">Stock</p><p className="font-bold text-stone-800">{p.stock} {p.unidad}</p></div><div><p className="text-stone-400">Mín</p><p className="font-bold text-stone-800">{p.stockMin}</p></div><div><p className="text-stone-400">Ventas/día</p><p className="font-bold text-stone-800">{p.ventasDiarias}</p></div><div><p className="text-stone-400">Se acaba en</p><p className={'font-bold ' + (p.diasParaAgotar <= 1 ? 'text-red-600' : 'text-stone-800')}>{p.diasParaAgotar.toFixed(1)} d</p></div></div>
            {p.diasParaAgotar <= 2 && <div className="mt-3 bg-amber-50 rounded-xl p-3 text-sm"><p className="font-medium text-amber-800">📋 Pedido sugerido: {Math.ceil(p.ventasDiarias * 7 - p.stock)} {p.unidad}</p><p className="text-xs text-amber-600">Proveedor: {p.proveedor}</p></div>}
          </div>
        ))}</div>
      </div>
      {showAdd && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}><h2 className="font-bold text-xl text-stone-900 mb-4">Agregar Producto</h2><div className="space-y-3"><input placeholder="Nombre" className="w-full p-3 bg-stone-50 border rounded-xl text-sm" /><div className="grid grid-cols-2 gap-2"><input type="number" placeholder="Precio" className="p-3 bg-stone-50 border rounded-xl text-sm" /><input type="number" placeholder="Stock" className="p-3 bg-stone-50 border rounded-xl text-sm" /></div></div><div className="flex gap-3 mt-4"><button onClick={() => setShowAdd(false)} className="flex-1 bg-stone-200 py-3 rounded-xl font-bold">Cancelar</button><button onClick={() => { alert('Producto agregado'); setShowAdd(false) }} className="flex-1 bg-emerald-500 text-white py-3 rounded-xl font-bold">Guardar</button></div></div>
        </div>
      )}
    </div>
  )
