...
export default function PedidosPage() {
  ...
  const searchParams = useSearchParams(); const tenantId = searchParams.get("tenant") || "7e045520-5e36-4e3f-a39f-10ea7d6dce76";
  ...
  const cambiarEstado = async (id: string, nuevoEstado: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMensaje(`Ã¢Å“â€¦ Estado actualizado a ${ESTADOS[nuevoEstado as keyof typeof ESTADOS]?.label || nuevoEstado}`);
  ...
  const confirmarPedido = async (id: string) => {
    if (!confirm("Ã‚Â¿Confirmar este pedido? Se descontarÃƒÂ¡ stock y se registrarÃƒÂ¡ in finanzas.")) return;
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMensaje("Ã¢Å“â€¦ Pedido confirmado correctamente.");
        setTimeout(() => setMensaje(""), 5000);
        cargarPedidos(true);
      } else {
        alert("Error: " + data.error);
  ...
  const cancelarPedido = async (id: string) => {
    if (!confirm('Ã‚Â¿EstÃƒÂ¡s seguro de cancelar este pedido? Se revertirÃƒÂ¡ inventario y finanzas.')) return;
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setMensaje('Ã¢Å“â€¦ Pedido cancelado exitosamente');
        setTimeout(() => setMensaje(''), 5000);
        cargarPedidos(true);
      } else {
        alert('Ã¢ÂÅ’ Error: ' + (data.error || 'Error al cancelar'));
      }
    } catch (error) {
      console.error(error);
      alert('Ã¢ÂÅ’ Error al cancelar pedido');
    }
  };
            ...
            {pedidosFiltrados.map((pedido) => {
              ...
              return (
                <div key={pedido.id} className="bg-white rounded-2xl p-4 shadow-sm border border-stone-200 hover:shadow-md transition flex flex-col">
                  ...
                  <p className="text-sm text-stone-500">Ã°Å¸â€œÂ¦ {pedido.items?.length || 0} productos</p>
                  <p className="text-sm text-stone-500">Ã°Å¸â€™Â° ${pedido.total?.toLocaleString()}</p>
                  <p className="text-xs text-stone-400">Pago: {pedido.metodo_pago}</p>
                  {pedido.direccion && <p className="text-xs text-stone-400">Ã°Å¸â€œÂ {pedido.direccion}</p>}
                  {pedido.observaciones && <p className="text-xs text-stone-400">Ã°Å¸â€œÂ {pedido.observaciones}</p>}

                  <div className="mt-2 text-xs text-stone-600 border-t pt-2">
                    {pedido.items?.slice(0, 2).map((item, idx) => (
                      <div key={idx} className="flex justify-between">
                        <span>{item.cantidad} Ãƒâ€” {item.nombre}</span>
                        <span>${(item.cantidad * item.precio).toLocaleString()}</span>
                      </div>
                    ))}
                    {pedido.items?.length > 2 && (
                      <div className="text-stone-400 text-xs mt-1">+ {pedido.items.length - 2} mÃƒÂ¡s</div>
                    )}
                  </div>
      ...
      {detallePedido && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            {detallePedido.direccion && <p className="text-sm text-stone-600">DirecciÃƒÂ³n: {detallePedido.direccion}</p>}
            {detallePedido.telefono && <p className="text-sm text-stone-600">TelÃƒÂ©fono: {detallePedido.telefono}</p>}
