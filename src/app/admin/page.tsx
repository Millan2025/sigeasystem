      ...
      {clienteDetalle && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setClienteDetalle(null)}>
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2 text-sm"><p><span className="text-stone-500">Tipo:</span> {clienteDetalle.tipo}</p><p><span className="text-stone-500">Gerente:</span> {clienteDetalle.gerente}</p><p><span className="text-stone-500">Email:</span> {clienteDetalle.email}</p><p><span className="text-stone-500">Telefono:</span> {clienteDetalle.telefono}</p><p><span className="text-stone-500">Plan:</span> <span className="font-bold text-emerald-600">{clienteDetalle.suscripcion}</span></p><p><span className="text-stone-500">Modulos:</span> {clienteDetalle.modulos.join(', ')}</p></div>
          </div>
        </div>
      )}
