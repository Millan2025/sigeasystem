...
export default function AdminMasterPage() {
  ...
  // ⚠️ NO hay returns tempranos - todo el JSX se renderiza condicionalmente
  ...
  // 🔥 FUNCIÓN PARA IMPORTAR PRODUCTOS DESDE ADMIN
            ...
            <form onSubmit={importarProductos} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  name="file" 
                  accept=".xlsx,.xls" 
                  className="w-full p-3 border-2 border-dashed border-stone-300 rounded-xl text-sm text-stone-800 bg-stone-50 focus:border-emerald-500 focus:outline-none file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100" 
                  required 
                />
                <p className="text-xs text-stone-500 mt-1">Usa la plantilla descargable desde el módulo Inventario del cliente.</p>
              </div>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => window.open('/api/admin/products/template', '_blank')}
                  className="flex-1 bg-blue-50 hover:bg-blue-100 text-blue-600 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-1 transition-all duration-200"
                >
                  <Download className="w-4 h-4" /> Descargar Plantilla
                </button>
              </div>
              {importProgress && (
                <div className={`p-3 rounded-xl text-sm font-medium ${importProgress.includes('✅') ? 'bg-emerald-50 text-emerald-700' : importProgress.includes('❌') ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700'}`}>
                  {importProgress}
                </div>
              )}
              <div className="flex gap-3 mt-4">
                ...
                <button 
                  ...
                  className={`flex-1 py-3 rounded-xl font-bold transition-all duration-200 ${isImporting ? 'bg-stone-400 text-white cursor-not-allowed' : 'bg-emerald-500 hover:bg-emerald-600 text-white'}`}
                >
                  {isImporting ? 'Subiendo...' : 'Subir'}
                </button>
              </div>
            </form>
          </div>
