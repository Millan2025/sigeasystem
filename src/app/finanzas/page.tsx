...
export default function FinanzasPage() {
  ...
  function calcularCierre() {
    ...
    if (Math.abs(diferencia) > efectivoEsperado * 0.02) {
      setResultadoCierre('Ã¢Å¡Â Ã¯Â¸Â DESCUADRE: Diferencia de $' + Math.abs(diferencia).toLocaleString() + ' (' + porcentaje + '%). Revisar ventas del dia.')
    } else {
      setResultadoCierre('Ã¢Å“â€¦ CUADRE CORRECTO: Diferencia de solo $' + Math.abs(diferencia).toLocaleString() + ' (' + porcentaje + '%). Todo en orden.')
    }
    setShowCierre(true)
  }
    ...
    <div className="min-h-screen bg-stone-50 max-w-lg lg:max-w-2xl mx-auto">
      <header className="bg-gradient-to-r from-stone-900 to-stone-800 text-white p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-xl font-bold">Ã°Å¸ÂÂ¦ Finanzas</h1>
            <p className="text-stone-400 text-sm">Panaderia DoÃƒÂ±a Rosa</p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl p-5 border border-stone-200">
              ...
              {showCierre && resultadoCierre && (
                <div className={'mt-4 p-4 rounded-2xl ' + (resultadoCierre.startsWith('Ã¢Å“â€¦') ? 'bg-emerald-50 border border-emerald-300' : 'bg-red-50 border border-red-300')}>
