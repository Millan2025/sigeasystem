...
const beneficiosPorModulo: Record<string, { titulo: string; icono: string; beneficios: string[]; color: string }> = {
  ...
  tienda: { titulo: "Tienda Online", icono: "ðŸ›’", beneficios: ["CatÃ¡logo actualizado", "BÃºsqueda", "Carrito", "Checkout"], color: "bg-orange-500" },
...
export default function NegocioHome({ negocioSlug, tenantId: tenantIdProp }: { negocioSlug?: string; tenantId?: string }) {
  ...
  const modulos = [
    ...
    { id: "produccion", label: "ProducciÃ³n", icon: ChefHat, color: "bg-lime-50 border-2 border-[#B8860B] text-lime-600", href: `/${negocioSlug || "restaurante"}/produccion?tenant=${tenantId}` },
