...
import { Upload, Calendar, CheckCircle, XCircle, Plus, Trash2, Clock, Bell, Share2, Smartphone, Settings } from "lucide-react";
...
export default function WhatsAppMarketing({ tenantId, negocioNombre }: Props) {
  ...
  const programar = async () => {
    ...
    if (error) { showToast("Error al programar: " + error.message, "error"); }
      ...
      showToast("✅ " + filas.length + " envío(s) programado(s)");
    ...
    <div className="max-w-4xl mx-auto p-4 space-y-6 w-full max-w-full overflow-x-hidden">
      ...
      {vencidas.length > 0 && (
        <div className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-3"><Bell className="w-5 h-5 text-amber-600" /><h3 className="font-bold text-amber-900">⏰ Es hora de publicar ({vencidas.length}) - envío automatico al dueno en curso</h3></div>
